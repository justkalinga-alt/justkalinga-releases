...
final class JKSH_Pinterest_Connector {
    ...
    public function readiness() {
        $c=$this->config(); $rows=$this->deps_ready()?$this->accounts()->connected(self::PLATFORM):[]; $verified=0; $boards=[];
        foreach($rows as $row){ $s=$this->accounts()->settings($row); if(!empty($s['profile_verified_by_api']))$verified++; if(empty($boards)&&!empty($s['boards'])&&is_array($s['boards']))$boards=$s['boards']; }
        $dry=$this->deps_ready()?(bool)$this->settings_service()->get('dry_run',1):true;
        $configured=''!==$c['app_id']&&''!==$c['app_secret'];
        return [
            'ok'=>true,'version'=>self::VERSION,'platform'=>self::PLATFORM,'api_version'=>'v5','configured'=>$configured,
            'api_access_confirmed'=>$c['api_access_confirmed'],'connected_accounts'=>count($rows),'verified_accounts'=>$verified,
            'boards_count'=>count($boards),'default_board_id'=>$c['default_board_id'],'dry_run'=>$dry,
            'pinterest_live_publish_enabled'=>$c['live_publish_enabled'],'standard_access_confirmed'=>$c['standard_access_confirmed'],
            'live_publish_possible'=>$configured&&$c['api_access_confirmed']&&$c['standard_access_confirmed']&&$verified>0&&count($boards)>0&&''!==$c['default_board_id']&&!$dry&&$c['live_publish_enabled'],
            'publisher_gate'=>($configured&&$c['api_access_confirmed']&&$c['standard_access_confirmed']&&$verified>0&&count($boards)>0&&''!==$c['default_board_id'])?'ready_to_arm':'locked',
            'redirect_uri'=>$this->redirect_uri(),'scope'=>$this->scopes(),'supported'=>['image_pin','video_pin'],
            'note'=>'Pinterest public production writes require Standard access, an API-verified account, a selected board, Dry Run OFF and the dedicated Pinterest live switch.'
        ];
    }
    ...
    private function preflight($v){ $a=$this->primary_account(); if(!$a)return ['ok'=>false,'message'=>'No API-verified Pinterest account is connected.']; $c=$this->config(); if(!$c['default_board_id'])return ['ok'=>false,'message'=>'Select a default Pinterest board.']; $ids=$this->media_ids($v); if(1!==count($ids))return ['ok'=>false,'message'=>'Pinterest publishing requires exactly one image or video attachment.']; $mime=(string)get_post_mime_type($ids[0]); if(!str_starts_with($mime,'image/')&&!str_starts_with($mime,'video/'))return ['ok'=>false,'message'=>'Pinterest supports image or video Pins only.']; if(str_starts_with($mime,'video/')){ $f=$this->fields($v); $cover=absint($f['cover_attachment_id']??0); if(!$cover)return ['ok'=>false,'message'=>'Pinterest video Pins require a cover image.']; }
        return ['ok'=>true,'message'=>'Pinterest preflight passed.']; }

    private function publish($v,$payload){ $pf=$this->preflight($v); if(!$pf['ok'])return ['ok'=>false,'retryable'=>false,'message'=>$pf['message']]; $a=$this->primary_account(); $token=$this->token_for($a); if(is_wp_error($token))return ['ok'=>false,'retryable'=>false,'message'=>$token->get_error_message()]; $c=$this->config(); $f=$this->fields($v); $ids=$this->media_ids($v); $id=$ids[0]; $mime=(string)get_post_mime_type($id); $title=$this->text_limit($f['title']??$v->title??'',100); $desc=$this->text_limit($f['description']??$v->description??$v->caption??'',800); $link=esc_url_raw((string)($f['link']??$f['product_url']??'')); $alt=$this->text_limit($f['alt_text']??'',500); $board=sanitize_text_field((string)($f['board_id']??$c['default_board_id'])); $body=['board_id'=>$board,'title'=>$title,'description'=>$desc]; if($link)$body['link']=$link; if($alt)$body['alt_text']=$alt;
    ...
    public function process_job($job_id){ if(!$this->deps_ready()||!class_exists('JKSH\\Repositories\\JobRepository'))return; $jobs=new \JKSH\Repositories\JobRepository(); $job=$jobs->get(absint($job_id)); if(!$job||self::PLATFORM!==sanitize_key((string)$job->platform)||in_array((string)$job->status,['published','dry_run_complete','cancelled','failed','blocked'],true))return; $variants=new \JKSH\Repositories\VariantRepository(); $v=$variants->get((int)$job->variant_id); if(!$v)return; $payload=json_decode((string)($job->payload_json??'{}'),true);$payload=is_array($payload)?$payload:[];$mode=sanitize_key((string)($job->publish_mode??'dry_run')); $now=current_time('mysql',true); $attempts=(int)$job->attempts+1; $jobs->update((int)$job->id,['status'=>'processing','attempts'=>$attempts,'started_at'=>$now]); $pf=$this->preflight($v);
        ...
        $c=$this->config(); if(!$c['live_publish_enabled']){$jobs->update((int)$job->id,['status'=>'blocked','last_error'=>'Pinterest live publishing master switch is OFF.','completed_at'=>$now]);$variants->update_status((int)$v->id,'blocked');return;}
        $res=$this->publish($v,$payload); if(!empty($res['ok'])){ $eid=sanitize_text_field((string)($res['external_id']??''));$eurl=esc_url_raw((string)($res['external_url']??''));$jobs->update((int)$job->id,['status'=>'published','payload_json'=>wp_json_encode($res['payload']??$payload),'last_response_json'=>wp_json_encode($res['response']??[]),'last_error'=>'','completed_at'=>current_time('mysql',true)]);$variants->update_status((int)$v->id,'published',['external_id'=>$eid,'external_url'=>$eurl]); (new \JKSH\Repositories\LogRepository())->add(['job_id'=>(int)$job->id,'content_id'=>(int)$job->content_id,'variant_id'=>(int)$job->variant_id,'action'=>'publish','result'=>'success','external_id'=>$eid,'external_url'=>$eurl,'message'=>(string)$res['message'],'context'=>['platform'=>self::PLATFORM]]); global $wpdb;$t=$wpdb->prefix.'jksh_jobs';$remaining=(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$t} WHERE content_id=%d AND id<>%d AND status NOT IN ('published','published_manual','cancelled')",(int)$job->content_id,(int)$job->id));if(0===$remaining)(new \JKSH\Repositories\ContentRepository())->update_status((int)$job->content_id,'published');return; }
    ...
    public function admin_footer(){ if(!is_admin()||'jksh-accounts'!==sanitize_key((string)($_GET['page']??''))||!$this->deps_ready())return; $c=$this->config();$r=$this->readiness();$accounts=$this->accounts()->connected(self::PLATFORM);$boards=[];if($accounts){$s=$this->accounts()->settings($accounts[0]);$boards=is_array($s['boards']??null)?$s['boards']:[];} ?>
      <section id="jksh-pinterest-foundation-v010" class="jksh-card" style="display:none">
        ...
        <p><strong>Pinterest v0.1.4:</strong> API v5 OAuth, continuous token refresh, board discovery, image/video Pin publishing, production access gating, queue interception and external read-back verification.</p>
